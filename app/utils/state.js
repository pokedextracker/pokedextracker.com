...
export function loadState () {
  ...
  const notif20220808 = localStorage.getItem('notif-2022.08.08') === 'true' || undefined;
  const token = localStorage.getItem('token');
  const session = tokenToUser(token);
  const showInfo = localStorage.getItem('showInfo') === 'true' || undefined;

  return { nightMode, notification: notif20220808, session, showInfo, token };
}
...
export function saveState ({ nightMode, notification, showInfo, token }) {
  ...
  localStorage.setItem('notif-2022.08.08', notification);
